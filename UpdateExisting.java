	...
	public class Update extends BaseClass {
		...
		public void update(String urgencyvalue, String statevalue) throws InterruptedException {
			...
			WebElement searchBox = driver.findElement(By
					.xpath("//span[contains(text(),'Press Enter from within the input to')]/following-sibling::input"));
			...
			Thread.sleep(2000);
			driver.findElement(By.xpath("//a[contains(text(),'INC')]")).click();
			String statevalue1 = driver
					.findElement(By.xpath("(//span[text()='Incident state'])[1]/following-sibling::span/span"))
					.getText();
			String priority = driver
					.findElement(By.xpath("(//span[text()='Priority'])[2]/following-sibling::span/span")).getText();
			if (statevalue.contains(statevalue1) && priority.contains(priority)) {
				System.out.println("Urgency as High and State as In Progress");

			} else {
				System.out.println("Urgency not High and State  as not In Progress");
			}
		}
